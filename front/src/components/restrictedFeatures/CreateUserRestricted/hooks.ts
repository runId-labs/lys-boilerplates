...
import CreateUserRestricted, {CreateUserMutation} from "./index";
import {PrimaryAction} from "@/components/features/ShowActionsFeature/types";

/**
 * Hook options
 */
interface UseCreateUserRestrictedActionOptions {
    /**
 ...
 * ```
 */
export function useCreateUserRestrictedAction(
    options?: UseCreateUserRestrictedActionOptions
): PrimaryAction | null {
    const intl = useIntl();
    ...
    const hasPermission = useMemo(() => {
        return checkOperationsPermission(CreateUserMutation);
    ...
    const handleOpen = useCallback(() => {
        open({
            uniqueKey: "create-user",
            title: intl.formatMessage({
                id: "lys.components.restrictedFeatures.createUserRestricted.title",
                defaultMessage: "Create User"
            }),
            body: CreateUserRestricted,
            bodyProps: {
                onCompleted: () => {
                    // Close dialog first
                    close();
